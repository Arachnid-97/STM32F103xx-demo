...
//    lv_table_set_cell_value(table, 3, 2, "ms");
    ...
    lv_bar_set_value(bar, 100, false);

    lv_obj_t * slider = lv_slider_create(h, NULL);
    lv_bar_set_value(slider, 100, false);
...
static void create_tab2(lv_obj_t * parent)
{
    ...
    lv_obj_align(arc, gauge, LV_ALIGN_OUT_BOTTOM_MID, 0, LV_DPI / 8 - 3);

    lv_obj_t * ta = lv_ta_create(parent, NULL);
    lv_obj_set_size(ta, w / 3 - 3, lv_disp_get_ver_res(NULL) / 4 - 3);
...
//	lv_obj_set_size(loader, w / 3 - 1, lv_disp_get_ver_res(NULL) / 4 - 3);
...
static void create_tab3(lv_obj_t * parent)
{
    ...
    lv_obj_set_size(win, lv_disp_get_hor_res(NULL) / 2 + 4, lv_disp_get_ver_res(NULL) / 2 + 1);
    lv_obj_set_pos(win, LV_DPI / 20 + 2, LV_DPI / 20 + 2);
    ...
    lv_obj_set_size(page, lv_disp_get_hor_res(NULL) / 3 + 2, lv_disp_get_ver_res(NULL) / 2 + 2);
    ...
    lv_obj_set_size(cal, 5 * LV_DPI / 2 + 8, 5 * LV_DPI / 2 + 8);
    lv_obj_align(cal, page, LV_ALIGN_OUT_RIGHT_TOP, -LV_DPI*2 / 2, LV_DPI*2 / 3);
