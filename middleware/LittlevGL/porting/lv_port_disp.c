    ...
    for(y = area->y1; y <= area->y2; y++) {
        for(x = area->x1; x <= area->x2; x++) {
            ...
            /* 
				...
				color_p++;
			*/
//			TFT_SetPointPixel(x, y, *buf);
//			buf++;
			TFT_SetPointPixel(x, y, color_p->full);
			color_p++;
        }
    }
	...
	for( ; size>0; size--)
	{
		ILI9341_FillColor(color_p->full, 1);
		color_p++;
	}

//	TFT_FillArea(act_x1, act_y1, act_x2, act_y2, *buf, size);
